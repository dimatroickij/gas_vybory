...
class UIP:
    ...
    def gen_doc_number(srlf, type):
        if type == 0:
            ...
            number = str(random.randint(1, 999999))
            number = '0' * (6 - len(number)) + number
            series = romanNum[random.randint(0, 4)] + '-' + alphabet[random.randint(0, 10)] + alphabet[
                random.randint(0, 10)]
        else:
            number = ''
            series = ''
    ...
    def gen_name(self, sex):
        return random.choice(self.peopleFile['names'][sex])
    ...
    def getElector(self):
        return [self.last_name, self.first_name, self.middle_name, self.birth_day, self.birth_day_string,
                self.is_birth_day_year_only, self.gender_id, self.capacity_id, self.country_id, self.snils, self.inn,
                self.start_date, self.end_date, self.input_source, self.is_actual, self.kca, self.capacity_actual_date,
                self.elector_id, self.imanid, self.sys_elector_id, self.address_id, self.ik_id_array,
                self.is_public_version]
    ...
    def getElector_doc(self):
        return [self.subject_global_id, self.elector_doc_code_id, self.elect_doc_series, self.elect_doc_number,
                self.input_source, self.start_date2, self.end_date, self.is_actual, self.kca, self.elect_doc_issue_date,
                self.elector_doc_type_id, self.elector_doc_id, self.sys_elector_id, self.ik_id_array,
                self.is_public_version]
    ...
    def getElector_residence(self):
        return [self.temp_start_date, self.temp_end_date, self.input_source, self.elector_residence_kind_id,
                self.start_date3, self.end_date, self.is_actual, self.kca, self.arrive_prefered_date,
                self.is_create_arrive_needed, self.temp_doc_number, self.elector_residence_id, self.sys_elector_id,
                self.residence_address_id, self.arrival_address_id, self.departure_address_id, self.ik_id_array,
                self.is_public_version]
    ...
    def getElector_change_log(self):
        return [self.elector_zags_info_id, self.change_type_id, self.change_number, self.change_date, self.is_actual,
                self.start_date2, self.create_user_id, self.input_source, self.kca, self.change_basis_id, self.is_death,
                self.is_deleted, self.import_log_id, self.is_approved, self.start_date2, self.update_user_id,
                self.is_departured_in_military, self.elector_change_log_id, self.elector_id, self.elector_doc_id,
                self.elector_residence_id, self.elector_zags_info_id, self.sys_elector_id, self.is_departured_from_addr,
                self.is_departured_in_prison, self.kind_names, self.is_checked_ukd, self.is_valid_ukd, self.is_trusted,
                self.ik_id_array, self.is_public_version]
